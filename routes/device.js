...
router.post("/register", async (req, res) => {
  ...
  logger.log(`[register_device] get token -- device_id: ${device_id}, device_type: ${device_type}, device_token: ${device_token}`);
...
router.post("/notification/:secret_key", async (req, res) => {
  if(!validateSecretKey(req.params.secret_key)) {
    logger.log(`[ipification_notification] wrong secret key: ${req.params.secret_key}`);
  ...
  if(!device_id) {
    logger.log(`[ipification_notification] device_id is required`);
    res.status(HTTP_STATUS.BAD_REQUEST).send(ERROR_MESSAGES.DEVICE_ID_REQUIRED);
    return;
  }

  if(!notification_type) {
    logger.log(`[ipification_notification] notification_type is required`);
  ...
  logger.log(`[ipification_notification] device_id: ${device_id}, notification_type: ${notification_type}, device_info: ${JSON.stringify(device_info)}`);

  if(!device_info) {
    logger.log(`[ipification_notification] device not found`);
    res.send();
    return;
  }

  try {
    logger.log(`[ipification_notification] invoke send_notification`);
    ...
    logger.error('[ipification_notification] error:', error);
