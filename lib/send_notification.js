...
const send_notification = async (device_info, title, body) => {
  ...
  logger.log('[send_notification] start send_notification');

  const message = buildFcmPayload(device_type, device_token, title, body);

  logger.log(`[send_notification] device_type: ${device_type}, device_token: ${device_token}`);
  logger.log(`[send_notification] message: ${JSON.stringify(message)}`);

  try {
    getFirebaseApp();
    const response = await admin.messaging().send(message);

    logger.log(`[send_notification] response: ${response}`);
    return response;
  } catch (error) {
    logger.error('[send_notification] error:', error.message);
    throw error;
  }
}
